...
const monteCarloCircleArea = ({
  ...
  let rectArea =
    Geo.get_distance(minLat, minLng, maxLat, minLng) *
  ...
  for (let i = 0; i < simCount; ++i) {
    ...
    if (Geo.get_distance(cLat, cLng, pLat, pLng) <= radius) inCircleCounts++;
  }
...
const lazyCompression = ({geoHashes, lat, lng, radius, error, precision}) => {
    ...
    if (vacantAreaPercentage > maxVacantAreaPercentage) {
      ...
      extraHashes.push(
        ...bboxes(
          ...
          (_hash) =>
            !Geo.is_hash_outside_circle(_hash, lat, lng, radius, error),
        ),
      );
    }
  }
  ...
  for (let hash of toDeleteHashes) geoHashes.delete(hash);
  //add new hashes
  for (let hash of extraHashes) geoHashes.add(hash);
