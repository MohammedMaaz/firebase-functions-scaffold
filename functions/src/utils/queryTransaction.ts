...
async function rerun() {
  try {
    this.uploadBatch = db().batch();
    this.rollbackBatch = db().batch();
    this.uploadRefs = [];
    this.rollbackRefs = [];
    this.writeInfo = {
    ...
    if (this.rollbacking) await rollback.call(this);
...
async function rollback() {
  try {
    if (this.rollbackRefs.length > 0) {
      ...
      await promiseWithRetry(this.rollbackBatch.commit(), 'medium');
      this.rollbacking = false;

      if (this.retries < this.maxRetries) {
        this.retries++;
        rerun.call(this);
      } else throw 'retries exceeded limit';
    } else throw 'side effect left due to concurrency';
  } catch (error) {
