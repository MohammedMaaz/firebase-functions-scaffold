...
const bucket = firebase_config_1.storage().bucket();
const paramsToUrl = (filePath, token) => {
    const BUCKET_NAME = `${firebase_config_1.functions.config().project.id}.appspot.com`;
...
const getDownloadUrl = async (filePath) => {
    try {
        ...
        const uuid = utils_1.randHashString(24);
...
const on_image_upload = async (object) => {
    try {
        ...
        const uploadHook = typeof metadata.uploadHook === "string"
            ? JSON.parse(metadata.uploadHook)
            : undefined;
        const variants = typeof metadata.resize === "string"
        ...
        if (uploadHook && !variants) {
            ...
            await cf_1.changeTaskStatus({
                docRef: firebase_config_1.refs.users.doc(uid),
        ...
        const [path, os, fs, sharp] = await utils_1.dynamicImport("path", "os", "fs-extra", "sharp");
        //constants
        ...
        for (let variant in variants) {
            ...
            let VariantName = `${utils_1.fileNameWithoutExtension(sourceName)}@${variant}_${variants[variant]}${path.extname(sourceName)}`;
            let variantPath = path.join(workingDir, VariantName);
            const area = variants[variant];
            let toDim = utils_1.reduceToArea({ width, height, area });
            //resizing image
            await sharp(tempFilePath)
                .resize(toDim.width, toDim.height)
                .toFile(variantPath);
            //pushing upload in promises array
            uploadPromises.push(uploadFile(variantPath, bucketDir + "/" + VariantName));
        }
